...
Notes:
  ...
  * When `r expression(R)` is a `n x n` matrix with a random sample of size `n` from a $\chi$(df=2) distribution on the diagonal (and 0 off-diagonal), RU is the bivariate normal distribution G(0,I) and appears as a roughly spherical scatterplot (`mvpd` uses the letter G to denote Gaussian).
  * `r expression(RUA)` is the result of the matrix multiplication between `n x 2` `r expression(RU)` and `2 x 2` `r expression(A)`, the Cholesky decomposition of a `2x2` matrix `r expression(Q)`.
  ...
  * RUA is the bivariate normal distribution G(0,Q) (`mvpd` uses the letter G to denote Gaussian).
  * Changing the distribution in R changes the multivariate distribution of `r expression(RUA)` .  That's what this package is all about!

You can change the order of application of `r expression(A)` and `r expression(R)` -- check out [the other intro](https://swihart.github.io/mvpd/articles/intro.html).
