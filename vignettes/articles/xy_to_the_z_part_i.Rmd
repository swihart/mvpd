...
Let's see what the resultant Z(log(1/X)+log(1/Y)) distribution would look like if we replaced `z<-runif(nsims, 0, 1)` with `z <- rep(1/2, nsims)` in the code. We will see it "concentrates" too much and doesn't recover the distribution of log(1/X).
...
Hypothesis: even though this Z will "1/2 on average" it fails to recover the distribution log(1/X) because values bigger than 1 fail to "tighten" the distribution. Compounding the problem is values below 0 (negative values) flip/reflect values across the y-axis.  See below:
...
hist(    z , freq=FALSE, breaks=50, ylim=s.ylim, xlim=c(-0.1,1.1), 
     main="Z", xlab="",ylab="")
...
It looks like we need more tightening.  Instead of multiplying by 1/2 on average, we need to multiply by 1/3 on average.  We need Z uniform, with positive values smaller than 1 that average to a 1/3.  Let's replace `z<-runif(nsims, 0, 1)` with `z<-runif(nsims, 0, 2/3)` in the code:
...
Doesn't quite seem to do it.  That's because the sum of 2 exponential is not an exponential, so certainly a sum of 3 exponentials is not going to be an exponential.  In fact, looking at the [inductive form](https://math.stackexchange.com/a/3485159/180716), Z may not have a "recovering" distribution for a sum of more than 2 exponentials.
...
One can tinker and see how close they can get but the theory says it will be in exact.
...
hist(z*(log(1/x)+log(1/y)+log(1/a)), freq=FALSE, breaks=500, ylim=s.ylim, xlim=s.xlim, 
     main="Z(log(1/X)+log(1/Y)+log(1/A))", xlab="",ylab="")
lines(seq(s.xlim[1], s.xlim[2],0.01 ),
      dexp(seq(s.xlim[1], s.xlim[2],0.01 ),
           aa_x),
