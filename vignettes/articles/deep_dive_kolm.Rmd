...
There are two (equivalent) ways to represent the Kolmogorov distribution, in what is sometimes called a "limiting form" - which here means it involves an infinite sum:
...
Since $K1$ is an alternating sum, if the number of terms is odd, we'll see that $K1$ produces aberrant that are positive, instead of negative.  However $K2$ gets close to 0 for $x=12$ but is still negative.
